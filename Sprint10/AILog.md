...
## AI Log 17: Implementacija mjesečnih izvještaja i PDF generisanja
...
**Svrha korištenja:**  
Implementacija funkcionalnosti za generisanje mjesečnih izvještaja o zaduživanjima, rezervacijama i članovima, uključujući eksport izvještaja u PDF formatu.

**Kratak opis zadatka ili upita:**  
Od AI alata je zatraženo da pomogne u implementaciji sistema izvještaja koji osoblju omogućava izbor tipa izvještaja, mjeseca i godine, nakon čega sistem generiše pregled podataka za odabrani period. Također je tražena pomoć pri odabiru biblioteke za PDF generisanje, prikazu statistike kroz grafikone i prilagođavanju izgleda PDF dokumenta postojećem dizajnu sistema.

**Šta je AI predložio ili generisao:**  
AI je predložio strukturu backend logike za filtriranje podataka po mjesecu i godini, validaciju odabranog perioda i generisanje različitih tipova izvještaja. Predložena je i integracija biblioteke `Chart.js` za prikaz grafikona i statističkih podataka, kao i implementacija PDF eksport funkcionalnosti za prikaz tabela, statistike i sažetaka izvještaja.

**Šta je tim prihvatio:**  
Tim je prihvatio implementaciju forme za izbor mjeseca i godine, logiku filtriranja podataka po periodu, generisanje PDF izvještaja i korištenje `Chart.js` grafikona za vizuelni prikaz statistike unutar izvještaja.

**Šta je tim izmijenio:**  
Tim je prilagodio izgled PDF dokumenta postojećem dizajnu SmartLib sistema, ručno korigovao stilove i raspored elemenata, te optimizovao prikaz grafikona i tabela kako bi bili pregledniji unutar PDF formata.

**Šta je tim odbacio:**  
Odbačeni su prijedlozi koji su zahtijevali kompleksnije biblioteke za generisanje PDF dokumenata.

**Rizici, problemi ili greške koje su uočene:**  
Glavni problemi odnosili su se na izbor odgovarajuće biblioteke za PDF generisanje i integraciju postojećih CSS stilova unutar PDF dokumenta, jer pojedini stilovi nisu bili pravilno renderovani prilikom eksportovanja. Također su uočeni problemi sa prikazom `Chart.js` grafikona unutar stranice.

## AI Log 18: Implementacija napredne pretrage i kombinovanih filtera
...
**Svrha korištenja:**  
Implementacija funkcionalnosti napredne pretrage i filtriranja knjiga po kategoriji, izdavaču i godini izdanja unutar SmartLib sistema.

**Kratak opis zadatka ili upita:**  
Od AI alata je zatraženo da pomogne u implementaciji sistema filtera koji korisnicima omogućava filtriranje knjiga po kategoriji, izdavaču i godini izdanja, kao i kombinovanje više filtera istovremeno radi preciznije pretrage kataloga knjiga. Također je tražena pomoć pri odabiru načina prikaza filtera na korisničkom interfejsu, odnosno da li koristiti posebni modal/prozor ili inline formu unutar stranice kataloga.

**Šta je AI predložio ili generisao:**  
AI je predložio backend logiku za kombinovanje više filtera, validaciju unesenih vrijednosti i ažuriranje liste knjiga na osnovu aktivnih filtera. Predložena su i rješenja za korisnički interfejs, uključujući modalni prikaz filtera i inline filter sekciju direktno iznad kataloga knjiga. Generisani su prijedlozi za prikaz poruka u slučaju da nema rezultata za odabrane kriterije.

**Šta je tim prihvatio:**  
Tim je prihvatio implementaciju kombinovanih filtera i logiku dinamičkog filtriranja knjiga po više kriterija istovremeno. Također je prihvaćen pristup sa inline filter formom unutar stranice kataloga jer omogućava brže i jednostavnije korištenje bez dodatnog otvaranja prozora.

**Šta je tim izmijenio:**  
Tim je prilagodio izgled filter sekcije postojećem dizajnu SmartLib sistema, optimizovao raspored filter polja i prilagodio validacione i statusne poruke korisniku.

**Šta je tim odbacio:**  
Odbačen je pristup sa posebnim modalnim prozorom za filtere jer je procijenjeno da nepotrebno usporava korisnički tok i otežava brzo pregledavanje kataloga knjiga.

**Rizici, problemi ili greške koje su uočene:**  
Glavni rizik bio je nepravilno kombinovanje više filtera, što je moglo dovesti do netačnih rezultata pretrage ili vraćanja prevelikog broja knjiga. Također su uočeni problemi sa očuvanjem odabranih vrijednosti filtera nakon osvježavanja stranice, zbog čega je bilo potrebno dodatno prilagoditi View modele i logiku prikaza filter forme.

## AI Log 19: Implementacija online produžetka članarine 
...
**Svrha korištenja:**
Implementacija funkcionalnosti online produženja članarine unutar SmartLib sistema.

**Kratak opis zadatka ili upita:**
Od AI alata je zatražena pomoć pri dizajnu i implementaciji sistema online produženja članarine. Tražena je analiza kako bibliotečki sistemi implementiraju ovakvu funkcionalnost, prijedlog arhitekture, te kompletna implementacija: model, repozitorij, DTOs, controller akcije i Razor viewovi za obje strane (član i bibliotekar).

**Šta je AI predložio ili generisao:**
Zahtjev koji bibliotekar odobrava ili odbija, kao prikladniji od automatskog produženja jer pruža kontrolu i revizijski trag. Generisani su: model `ZahtjevProduzenja`, interfejs i repozitorij sa audit logom, pet DTO/ViewModel klasa, četiri nove controller akcije, te dvije Razor stranice — `ProduzenjeClanarine.cshtml` za člana i `ZahtjeviProduzenja.cshtml` za bibliotekara — uz CSS koji prati postojeći dizajn. AI je tokom sesije rješavao i tehničke probleme vezane za Docker migracije, build greške i cache probleme.

**Šta je tim prihvatio:**
Tim je prihvatio kompletnu strukturu modela, logiku izračuna novog datuma isteka, live preview pri odabiru trajanja i historiju zahtjeva s razlogom odbijanja vidljivom članu.

**Šta je tim izmijenio:**
Viewovi su kroz više iteracija vizualno prilagođavani stilu aplikacije korištenjem postojećih CSS komponenti (`izvjestaj-card`, `prod2-*` klase).

**Šta je tim odbacio:**
Odbačen je Model A (automatsko produženje) i implementacija notifikacija pri obradi zahtjeva — planirano za naredni sprint.

**Rizici, problemi ili greške koje su uočene:**
Kod je deployovan na Render prije pokretanja EF migracije, što je uzrokovalo grešku `Table 'ZahtjeviProduzenja' doesn't exist`. Problem je privremeno riješen ručnim SQL skriptom u Docker kontejneru. Dodatno, AI je u jednoj verziji koristio neispravnu Razor sintaksu za komentare (`{{!-- --}}` umjesto `@* *@`), što je uzrokovalo build grešku koja je uočena iz Docker loga.

## AI Log 20: Implementacija audit log promjena
...
**Svrha korištenja:**
Implementacija funkcionalnosti automatskog evidentiranja promjena nad knjigama i korisničkim nalozima unutar SmartLib sistema (PB-46, US-91, US-92).

**Kratak opis zadatka ili upita:**
Od AI alata je zatražena pomoć u implementaciji kompletnog audit log sistema — model, repozitorij, izmjene kontrolera (`KnjigaController`, `KorisnikController`, `AdminController`), admin view za pregled zapisa s filterima i diff prikazom, te CSS stilizacija prilagođena postojećem dizajnu. Naglašen je zahtjev da zapisi budu dostupni samo administratoru i da nisu podložni izmjenama.

**Šta je AI predložio ili generisao:**
`IAuditLogRepository` i `AuditLogRepository` s paginacijom i filterima, izmjene svih relevantnih controller akcija s audit log pozivima, `AuditLog.cshtml` view s tabelom, filterima (entitet, akcija, datumski period) i inline diff prikazom vrijednosti prije/nakon izmjene, te odgovarajući CSS. Tokom sesije AI je rješavao i tehničke probleme — EF Core tracking bug, vremensku zonu i merge konflikte u `site.css`.

**Šta je tim prihvatio:**
Tim je prihvatio kompletnu strukturu repozitorija, audit log pozive u svim CRUD akcijama, vlastiti `AuditLog.cshtml` s Base64 enkodiranjem JSON vrijednosti u `data-` atribute i JavaScript diff prikazom na klijentskoj strani, te UTC+2 fiksni offset za prikaz vremena.

**Šta je tim izmijenio:**
Korišten je engleski naziv akcija (`CREATE`, `UPDATE`, `DELETE`) umjesto bosanskog. View je prilagođen da koristi `audit-*` CSS klase iz postojeće grane umjesto `al-*` klasa koje je AI generisao u kasnijim iteracijama.

**Šta je tim odbacio:**
Odbačeni su kasniji viewovi koje je AI generisao jer je tim već imao vlastiti funkcionalni view. Odbačen je i `TimeZoneInfo` helper — zamijenjen jednostavnijim fiksnim offsetom.

**Rizici, problemi ili greške koje su uočene:**
Najznačajniji tehnički problem bio je EF Core tracking bug — vrijednosti "prije izmjene" bile su identične vrijednostima "nakon" jer je serijalizacija rađena na već izmijenjenom tracked objektu. Riješeno kopiranjem vrijednosti u lokalne primitivne varijable prije izmjene. Dodatno, merge konflikti između `feature/audit-log` i `main` grane ostavili su nezatvorene CSS blokove u `site.css` koji su kvarili stilizaciju stranice, što je zahtijevalo ručnu analizu i ispravku fajla.

## AI Log 21: Integracija sa distributerom knjiga
...
**Svrha korištenja:**
Implementacija funkcionalnosti slanja zahtjeva za nabavku knjiga distributeru direktno iz sistema.

**Kratak opis zadatka ili upita:**
Od AI alata je zatražena pomoć u implementaciji kompletne integracije sa distributerom — model, repozitorij, controller, view i CSS stilizacija. Dodatno je zatraženo proširenje funkcionalnosti desne kolone viewa: prikaz historije zadnjih nabavki i mogućnost promjene email adrese distributera bez izmjene koda.

**Šta je AI predložio ili generisao:**
`NabavkaZahtjev` i `AppPostavka` modele, `INabavkaRepository` s metodama za kreiranje, dohvatanje zadnjih nabavki i upravljanje email adresom distributera, `NabavkaController` s GET/POST akcijama za slanje zahtjeva i promjenu emaila, `Zahtjev.cshtml` view s formom, historijom i email panelom, odgovarajući CSS prilagođen postojećem dizajnu (`prod3-`, `zah3-`, `nabavka-` klase), te SQL blokove za kreiranje tabela u `Program.cs`.

**Šta je tim prihvatio:**
Tim je prihvatio kompletnu strukturu — modele, repozitorij, controller i view. Prihvaćen je i pristup kreiranja tabela kroz `ExecuteSqlRaw` u `Program.cs` umjesto EF Core migracija, što je konzistentno s načinom rada ostatka projekta. Email adresa distributera se čuva u `AppPostavke` tabeli i može se mijenjati direktno kroz UI.

**Šta je tim izmijenio:**
Property `PodnosiocId` preimenovan je u `PodnosilacId` radi konzistentnosti s bosanskim jezičkim pravilima — neusklađenost je inicijalno uzrokovala 500 grešku na produkciji.

**Šta je tim odbacio:**
Odbačen je info panel "Kako funkcioniše" iz desne kolone viewa — zamijenjen praktičnijim prikazom historije zadnjih nabavki i panelom za upravljanje email adresom distributera.

**Rizici, problemi ili greške koje su uočene:**
Neusklađenost između naziva propertija u modelu (`PodnosiocId`) i kolone u bazi (`PodnosilacId`) uzrokovala je `DbUpdateException` na produkciji. Dodatno, novi CSS blokovi su bili zarobljeni unutar nezatvorene `@media` zagrade u `site.css` nastaloj iz prethodnih merge konflikata, zbog čega stilovi nisu bili primijenjeni — riješeno analizom i ispravkom broja zagrada u fajlu.