...
- **Repository testovi** — testiraju poslovnu logiku direktno nad repozitorijima sa in-memory bazom podataka
...
* **Evidentiranje zaduživanja i vraćanja knjiga:**
    Nakon implementacije funkcionalnosti zaduživanja i vraćanja knjiga, izvršeno je regresiono testiranje kako bi se potvrdilo da nove operacije ne utiču na postojeću logiku upravljanja knjigama i korisnicima.

* **Rezervacije i automatsko isticanje (Sprint 9):**
    Nakon implementacije logike automatskog otkazivanja rezervacija (PB-43), izvršena je regresija nad modulom zaduživanja kako bi se potvrdilo da promjena statusa rezervacije ne narušava status primjeraka ni aktivna zaduženja. Potvrđeno je da `CancelExpiredAsync` ispravno ažurira samo rezervacije čiji je rok istekao, bez neželjenih nuspojava.
...
**Rezultati testiranja:**
Svi scenariji su testirani kroz UI (browser) i validirani očekivani ishodi. Manuelnim UAT testiranjem potvrđeno je da implementirane funkcionalnosti ispunjavaju sve definisane Acceptance Criteria iz Sprint Backloga.
...
**Pristup:** Integracijski testovi pokreću aplikaciju kroz stvarni HTTP pipeline (routing, model binding, validacija, autorizacija i pristup bazi) bez mockovanja kritičnih slojeva.

**Podjela:** Integracijski testovi su grupisani po funkcionalnim cjelinama:
- **Autentifikacija i autorizacija (RBAC)**
- **Korisnici i registracija**
- **Kategorije, knjige i primjerci**
- **Zaduženja i vraćanje knjiga**
- **Rezervacije - isticanje i izvještaji (Sprint 9)**
...
**Pristup:** UI testovi su izvođeni kroz automatizirane korisničke scenarije u browseru.
...
* **Opis:** Prilikom poziva `/api/primjerak/{id}`, polje `knjiga` (naslov) je vraćalo `null`, iako je `KnjigaId` bio ispravan.
* **Rješenje:** U `PrimjerakController` klasi, unutar LINQ upita, dodana je metoda `.Include(p => p.Knjiga)`.
* **Status:** Riješeno

### **BG-04: Email normalizacija pri registraciji i prijavi**
* **Opis:** Korisnik registrovan sa emailom `Korisnik@SmartLib.ba` mogao se prijaviti samo tim emailom — unos `korisnik@smartlib.ba` vraćao je `401 Unauthorized`.
* **Rješenje:** Dodan `.ToLower()` na email polje u register i login logici.
* **Status:** Riješeno

### **BG-05: Rezervacija dozvoljavala kreiranje kada knjiga ima dostupne primjerke**
* **Opis:** Forma za rezervaciju nije provjeravala dostupnost primjeraka na klijentskoj strani, što je omogućavalo kreiranje rezervacije za dostupnu knjigu.
* **Rješenje:** Dodata server-side validacija u `RezervacijaController` koja provjerava broj dostupnih primjeraka prije kreiranja rezervacije. Prikazuje se greška ako knjiga ima dostupne primjerke.
* **Status:** Riješeno

### **BG-06: Blokirana riječ u forum komentaru — manjkava povratna informacija**
* **Opis:** Kada bi korisnik pokušao objaviti komentar sa blokiranom riječju, sistem nije prikazivao jasnu poruku o razlogu odbijanja.
* **Rješenje:** Implementirana validacija u `ForumController.PostComment` koja vraća `BadRequest` sa jasnom porukom kada komentar sadrži zabranjeni sadržaj.
* **Status:** Riješeno
...
|  ID   | Naziv testa                                              | Opis                                            | Testni koraci                                                                  | Očekivani rezultat           | Stvarni rezultat        | US           | Status |
| :---: | :------------------------------------------------------- | :---------------------------------------------- | :----------------------------------------------------------------------------- | :--------------------------- | :---------------------- | :----------- | :----- |
|   1   | GetByKnjiga_KnjigaPostoji_VracaOkSaListomPrimjeraka      | Vraća listu svih primjeraka za postojeću knjigu | 1. Pozvati API sa validnim knjigaId                                            | 200 OK + lista primjeraka    | 200 OK + lista vraćena  | US-22, US-23 | Prošao |
...
#### 4.1.2 Repository testovi — Unit testovi (Sprint 9)

##### Rezervacija Repository (`RezervacijaRepositoryTests`)

Ovi unit testovi pokrivaju poslovnu logiku direktno nad `RezervacijaRepository` sa in-memory bazom podataka, bez HTTP sloja. Testiraju se ključne operacije istjecanja rezervacija i njihovog ispunjenja pri zaduživanju.

|  ID   | Naziv testa                                              | Opis                                                             | Testni koraci                                                                                    | Očekivani rezultat               | Stvarni rezultat              |  US   | Status |
| :---: | :------------------------------------------------------- | :--------------------------------------------------------------- | :----------------------------------------------------------------------------------------------- | :------------------------------- | :---------------------------- | :---: | :----- |
|   1   | CancelExpiredAsync_ExpiredReservation_IsMarkedIstekla    | Provjera da rezervacija čiji je rok prošao dobija status istekla | 1. Kreirati rezervaciju sa `DatumIsteka` u prošlosti (status = "aktivna")<br>2. Pozvati `CancelExpiredAsync()`<br>3. Učitati rezervaciju iz baze | Status rezervacije je "istekla"  | Status postavljen na "istekla" | US-79, US-80 | Prošao |
|   2   | FulfillAsync_ExistingActive_ChangesToRealizovana         | Provjera da aktivna rezervacija postaje realizovana pri zaduživanju | 1. Kreirati aktivnu rezervaciju za člana i knjigu<br>2. Pozvati `FulfillAsync(korisnikId, knjigaId)`<br>3. Učitati rezervaciju iz baze | Status rezervacije je "realizovana" | Status postavljen na "realizovana" | US-80 | Prošao |

---

#### 4.1.3 Web Kontroleri — Unit testovi
...
|  17   | Deaktiviraj_Uspjesno_PrimjerakViseNijeUBrojuDostupnih  | Deaktivirani primjerak isključen iz broja dostupnih   | Provjeriti dostupnost nakon deaktivacije           | Primjerak nije u dostupnim                   | Prošao           | US-24 | Prošao |
...
##### Vijest Web (`VijestWebControllerTests`) — Sprint 9

Ovi unit testovi pokrivaju MVC tokove za modul vijesti i obavještenja implementiran u Sprintu 9 (PB-57: US-110, US-111, US-112).


|  ID   | Naziv testa                                   | Opis                                                        | Testni koraci                                                         | Očekivani rezultat                                         | Stvarni rezultat | US     | Status |
| :---: | :-------------------------------------------- | :---------------------------------------------------------- | :-------------------------------------------------------------------- | :--------------------------------------------------------- | :--------------- | :----- | :----- |
|   1   | Index_ReturnsViewWithNews                     | Pregled liste objavljenih vijesti                           | Pozvati `Index()` akciju                                              | `ViewResult` sa listom vijesti                             | Kao očekivano    | US-110 | Prošao |
|   2   | Create_Post_Valid_SetsAutorAndRedirects       | Administrator dodaje vijest — postavljanje autora i redirect | Pozvati POST `Create()` sa validnim podacima (naslov, sadržaj, datum) | Autor se ispravno postavlja, redirect na listu vijesti     | Kao očekivano    | US-112 | Prošao |
|   3   | Create_Post_InvalidTitle_ReturnsBadRequestForAjax | Validacija obaveznog naslova vijesti                   | Pozvati POST `Create()` bez naslova vijesti                           | `BadRequest` ili View sa validacijskom greškom             | Kao očekivano    | US-112 | Prošao |

---

##### Kalendar Web (`KalendarWebControllerTests`) — Sprint 9

Ovi unit testovi pokrivaju MVC tokove za modul kalendara događaja implementiran u Sprintu 9 (PB-58: US-113, US-114, US-115).


|  ID   | Naziv testa                                    | Opis                                                       | Testni koraci                                                          | Očekivani rezultat                                        | Stvarni rezultat | US     | Status |
| :---: | :--------------------------------------------- | :--------------------------------------------------------- | :--------------------------------------------------------------------- | :-------------------------------------------------------- | :--------------- | :----- | :----- |
|   1   | Index_ReturnsAllEvents                         | Pregled svih događaja u kalendaru                          | Pozvati `Index()` akciju                                               | `ViewResult` sa listom svih događaja                      | Kao očekivano    | US-113, US-114 | Prošao |
|   2   | Create_Post_Valid_SetsAutorAndReturnsRedirect  | Administrator dodaje događaj — postavljanje autora i redirect | Pozvati POST `Create()` sa validnim podacima (naziv, datum, lokacija) | Autor se ispravno postavlja, redirect na kalendar         | Kao očekivano    | US-115 | Prošao |

---

##### Izvještaj Web (`IzvjestajWebControllerTests`) — Sprint 9

Ovi unit testovi pokrivaju MVC tok generisanja administrativnih izvještaja implementiran u Sprintu 9 (PB-45: US-87, US-88, US-89, US-90).


|  ID   | Naziv testa                                    | Opis                                                                  | Testni koraci                                                                          | Očekivani rezultat                                      | Stvarni rezultat | US           | Status |
| :---: | :--------------------------------------------- | :-------------------------------------------------------------------- | :------------------------------------------------------------------------------------- | :------------------------------------------------------ | :--------------- | :----------- | :----- |
|   1   | PregledRezervacija_Valid_ReturnsViewWithData   | Generisanje izvještaja o rezervacijama za odabrani period             | Pozvati akciju sa odabranim mjesecom i godinom za tip "Izvještaj o rezervacijama"      | `ViewResult` sa ispravnim podacima o rezervacijama za taj period | Kao očekivano | US-87, US-89 | Prošao |

---

##### Forum Web (`ForumWebControllerTests`) — Sprint 9

Ovi unit testovi pokrivaju MVC tokove za forum zajednice implementiran u Sprintu 9 (PB-59: US-116, US-117, US-118, US-119). Poseban fokus je na filtriranju zabranjenog sadržaja i ispravnoj obradi grešaka.


|  ID   | Naziv testa                            | Opis                                                                   | Testni koraci                                                              | Očekivani rezultat                                          | Stvarni rezultat | US     | Status |
| :---: | :------------------------------------- | :--------------------------------------------------------------------- | :------------------------------------------------------------------------- | :---------------------------------------------------------- | :--------------- | :----- | :----- |
|   1   | PostComment_BlockedWord_ShowsError     | Sistem odbija komentar koji sadrži zabranjenu/blokiranu riječ          | Pokušati objaviti komentar sa zabranjenom riječju putem `PostComment()` akcije | `BadRequest` ili View sa jasnom porukom o odbijanju        | Kao očekivano    | US-118 | Prošao |
|   2   | Details_NotFound_ReturnsNotFound       | Prikaz detalja nepostojeće forum teme vraća odgovarajući HTTP status   | Pozvati `Details(id)` sa nepostojećim ID-om teme                           | `NotFoundResult`                                           | Kao očekivano    | US-116 | Prošao |
...
| UAT-07 | Prijava sa pogrešnom lozinkom      | 1. Unijeti pogrešnu lozinku za već registrovanog člana                                                                                                                               | Generička poruka: Prijava nije uspjela.                                                                       |
...
| UAT-09 | Odjava                             | 1. Kliknuti na dugme 'Logout'<br>2. Pokušaj pristupa zaštićenoj stranici                                                                                                             | Redirect na login stranicu                                                                                    |
...
| UAT-11 | ISBN validacija                    | 1. Otvoriti formu klikom na dugme 'Nova knjiga'<br>2. Unijeti već postojeći ISBN<br>3. Sačuvati klikom na dugme 'Dodaj u katalog'                                                    | Ispis poruke: Knjiga sa ovim ISBN-om već postoji u katalogu.                                                  |
| UAT-12 | Broj primjeraka                    | 1. Unijeti broj (1 ili više)                                                                                                                                                         | Primjerci kreirani i prikazani u detaljima knjige                                                             |
| UAT-13 | Uređivanje knjige                  | 1. Ući na detalje jedne knjige iz kataloga<br>2. Kliknuti na dugme 'Uredi'<br>3. Unijeti izmjene<br>4. Kliknuti na dugme 'Sačuvaj izmjene'                                           | Promjene vidljive                                                                                             |
| UAT-14 | Brisanje knjige                    | 1. Ući na detalje jedne knjige iz kataloga<br>2. Kliknuti na dugme 'Obriši'<br>3. Potvrditi brisanje                                                                                 | Knjiga uklonjena                                                                                              |
| UAT-15 | Dodavanje primjeraka               | 1. Otvoriti detalje jedne knjige<br>2. Kliknuti na dugme 'Dodaj primjerak'<br>3. Unijeti željeni broj<br>4. Kliknuti na dugme 'Dodaj'                                                | Kreirani novi primjerci                                                                                       |
| UAT-16 | Dodavanje kategorije               | 1. Kliknuti na dugme 'Dodaj kategoriju' u sekciji Kategorija<br>2. Unijeti naziv i opis (opcionalno)<br>3. Kliknuti na dugme 'Sačuvaj'                                                | Nova kategorija se pojavljuje u listi kategorija                                                              |
...
| UAT-21 | Brisanje u upotrebi                | 1. Kliknuti na dugme 'Obriši' za kategoriju koja ima knjige                                                                                                                          | Greška: Kategorija 'X' ima Y knjiga i ne može biti obrisana.                                                  |
...
| UAT-30 | Pregled vlastitih rezervacija (Član)                 | 1. Prijava kao Član<br>2. Otvoriti sekciju 'Moje rezervacije'                                                                                                                      | Prikazuje se lista samo rezervacija prijavljenog člana sa naslovom knjige i datumom rezervacije                  |
| UAT-31 | Otkazivanje rezervacije (Član)                       | 1. Prijava kao Član<br>2. Otvoriti sekciju 'Moje rezervacije'<br>3. Kliknuti na dugme 'Otkaži' za jednu rezervaciju<br>4. Potvrditi otkazivanje                                   | Rezervacija uklonjena iz liste aktivnih rezervacija, sistem potvrđuje promjenu                                   |
| UAT-32 | Pregled aktivnih rezervacija (Bibliotekar)           | 1. Prijava kao Bibliotekar<br>2. Otvoriti sekciju 'Rezervacije'                                                                                                                    | Prikazuje se lista svih aktivnih rezervacija sa imenom člana, emailom, naslovom knjige i datumom rezervacije     |
| UAT-33 | Pretraga rezervacija po imenu člana (Bibliotekar)    | 1. Prijava kao Bibliotekar<br>2. Otvoriti sekciju 'Rezervacije'<br>3. Unijeti ime člana u polje pretrage<br>4. Kliknuti 'Traži'                                                   | Prikazuju se samo rezervacije koje odgovaraju unesenom imenu                                                     |
| UAT-34 | Automatsko otkazivanje isteklih rezervacija          | 1. Nakon isteka definisanog roka rezervacije (sistem automatski pokreće provjeru)<br>2. Provjeriti status rezervacije čiji je rok prošao                                          | Status rezervacije promijenjen u 'istekla', rezervacija više nije u listi aktivnih                               |
| UAT-35 | Filtriranje knjiga po kategoriji                     | 1. Otvoriti katalog knjiga<br>2. Izabrati filter 'Kategorija' i odabrati jednu kategoriju                                                                                          | Prikazuju se samo knjige iz odabrane kategorije; ako nema knjiga, prikazuje se poruka 'Nema rezultata'           |
| UAT-36 | Filtriranje knjiga po izdavaču                       | 1. Otvoriti katalog knjiga<br>2. Izabrati filter 'Izdavač' i odabrati jednog izdavača                                                                                              | Prikazuju se samo knjige odabranog izdavača; ako nema, prikazuje se poruka 'Nema knjiga za odabranog izdavača'   |
| UAT-37 | Filtriranje knjiga po godini izdanja                 | 1. Otvoriti katalog knjiga<br>2. Unijeti godinu u filter 'Godina izdanja'                                                                                                           | Prikazuju se samo knjige iz unesene godine; ako nema, prikazuje se poruka 'Nema knjiga za odabranu godinu'       |
| UAT-38 | Kombinovanje više filtera istovremeno                | 1. Otvoriti katalog knjiga<br>2. Odabrati kategoriju, izdavača i godinu izdanja istovremeno                                                                                        | Prikazuju se samo knjige koje zadovoljavaju sve odabrane filtere; ako nema, prikazuje se 'Nema rezultata'        |
| UAT-39 | Generisanje izvještaja o zaduživanjima               | 1. Prijava kao Bibliotekar/Administrator<br>2. Otvoriti sekciju 'Generiši izvještaj'<br>3. Odabrati 'Izvještaj o zaduživanjima'<br>4. Odabrati mjesec i godinu<br>5. Potvrditi     | Prikazuje se izvještaj sa ukupnim brojem zaduženja i listom zaduženih knjiga i članova za odabrani period        |
| UAT-40 | Generisanje izvještaja o rezervacijama               | 1. Prijava kao Bibliotekar/Administrator<br>2. Otvoriti sekciju 'Generiši izvještaj'<br>3. Odabrati 'Izvještaj o rezervacijama'<br>4. Odabrati mjesec i godinu<br>5. Potvrditi    | Prikazuje se izvještaj sa brojem aktivnih i završenih rezervacija, te listom rezervisanih knjiga i članova       |
| UAT-41 | Generisanje izvještaja o članovima                   | 1. Prijava kao Bibliotekar/Administrator<br>2. Otvoriti sekciju 'Generiši izvještaj'<br>3. Odabrati 'Izvještaj o članovima'<br>4. Odabrati mjesec i godinu<br>5. Potvrditi         | Prikazuje se izvještaj sa ukupnim brojem aktivnih članova za odabrani period                                     |
| UAT-42 | Pregled vijesti i obavještenja                       | 1. Otvoriti sekciju 'Vijesti' (dostupno svim korisnicima)                                                                                                                           | Prikazuje se lista objavljenih vijesti sa naslovom, kratkim opisom i datumom objave                              |
| UAT-43 | Dodavanje vijesti (Administrator)                    | 1. Prijava kao Administrator<br>2. Otvoriti formu za dodavanje vijesti<br>3. Unijeti naslov, kratak opis, puni sadržaj, datum objave i kategoriju<br>4. Sačuvati                  | Vijest se prikazuje u listi vijesti i dostupna je svim korisnicima na stranici 'Vijesti'                         |
...
#### 4.4.7 Rezervacija — istjecanje i izvještaji (`RezervacijaExpirationAndReportsIntegrationTests`) — 4 testa — Sprint 9

Ovi integracijski testovi validiraju kompletni životni ciklus rezervacija i funkcionalnost generisanja izvještaja kroz stvarni servisni i repozitorij sloj sa in-memory bazom podataka (bez HTTP pipeline-a).


|    ID     | Naziv testa                                                        | Šta validira                                                                                                   | Status |
| :-------: | :----------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------- | :----- |
| REZ-IT-01 | CancelExpiredAsync_ExpiredReservation_BecomesIstekla               | Rezervacija sa isteklim rokom (`DatumIsteka` u prošlosti) dobija status "istekla" nakon poziva `CancelExpiredAsync()` | Prošao |
| REZ-IT-02 | FulfillAsync_ExistingActiveReservation_BecomesRealizovana          | Aktivna rezervacija postaje "realizovana" nakon poziva `FulfillAsync(korisnikId, knjigaId)` — simulira preuzimanje knjige | Prošao |
| REZ-IT-03 | GenerirajMjesecneRezervacije_ReturnsExpectedCounts                 | Servis `IzvjestajService.GenerirajMjesecneRezervacije(5, 2025)` ispravno broji ukupne, aktivne i otkazane rezervacije za maj 2025 | Prošao |
| REZ-IT-04 | GenerirajMjesecnaZaduzenja_ReturnsExpectedCounts                   | Servis `IzvjestajService.GenerirajMjesecnaZaduzenja(5, 2025)` ispravno broji ukupna, aktivna i zatvorena zaduženja za maj 2025 | Prošao |
...
| AUTH-UI-04 | Login_Librarian_RedirectsToUserManagement              | Redirect bibliotekara        | Prošao |
| AUTH-UI-05 | Logout_ClearsSessionAndRedirectsToLogin                | Odjava i redirect na login   | Prošao |
